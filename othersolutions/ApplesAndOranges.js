...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
    var applesInsideHouse = 0;
    var orangesInsideHouse = 0;
    for (var i = 0; i < apples.length; i++){
        var position = a + apples[i];
        if (position >= s && position <= t) applesInsideHouse++;
    } 
    for (var i = 0; i < oranges.length; i++){
        var position = b + oranges[i];
        if (position >= s && position <= t) orangesInsideHouse++;
    } 
